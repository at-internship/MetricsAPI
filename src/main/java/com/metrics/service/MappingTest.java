...
public class MappingTest {
	...
	public boolean MappingTestMetric(CreateMetricRequest metric) {
		...
		try {
			...
			json = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(SetDefaultDataEmptyField(metric));
			...
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "invalid json data structure");
		}
	...
	private CreateMetricRequest SetDefaultDataEmptyField(CreateMetricRequest metric) {
		...
		MetricsApplication.logger
				.info("All data validation and default setting done returning a TRUE bool from the test");
		return collection;
	}
}